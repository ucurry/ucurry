...
let findFunctionType (name: string) (env: type_env) =
  match (findType name env)
  with FUNCTION_TY (argty, retty) -> (argty, retty)
...
let rec legalPattern (ty_env : type_env) (tau : typ)  (pat : pattern) = 
  ...
  | CON_PAT (name, Some pats) -> 
    ...
    in if (eqType (tau, ret_tau)) then 
      ...
      | (TUPLE_TY types, _) -> List.concat (List.map2 (legalPattern ty_env) types pats)
...
let rec typ_of (ty_env : type_env) (exp : Ast.expr)  = 
  let rec ty = function 
    ...
    | Let (bindings, e) ->
        ...
        if sameTypes then typ_of newEnv e else raise (TypeError "binding types are not annotated correctly")
    | Begin([]) -> UNIT_TY
    | Begin(es) -> 
    ...
    | Lambda (ty, formals, body) ->
        let rec check_lambda tau formals env = (match 
          (tau, formals) with
        | (_, []) -> if eqType (tau, typ_of env body) 
...
let rec type_def (def : Ast.def) (ty_env: type_env)  = 
  let ty = function 
    ...
    | Exp e ->  
        let _ = typ_of ty_env e in ty_env
    | CheckTypeError d -> 
        (try let _ = type_def d ty_env 
