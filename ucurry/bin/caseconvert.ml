...
let rec case_convert (ret_tau: A.typ) (scrutinee: S.sexpr) 
                 (cases: S.scase_expr list) (default: S.sexpr)
: S.sexpr  = 
...
| (pat, e) :: rest ->
  (match pat with
    | VAR_PAT name -> (ret_tau, S.SLet([((scrutinee_type, name), scrutinee)],e))
    | WILDCARD -> e
    | NIL -> (ret_tau, 
              S.SIf ((A.BOOL_TY, S.SUnop (A.IsNull, scrutinee)), 
              e, 
              continue_covert rest))
    | CONCELL (hd, tl) -> 
        let subtype = list_subtype scrutinee_type in 
        let hd_binding = ((subtype, hd), (subtype, S.SUnop (A.Hd, scrutinee))) in 
        let tl_binding = ((scrutinee_type, tl), (scrutinee_type, S.SUnop (A.Tl, scrutinee))) in 
