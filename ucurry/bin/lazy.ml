...
let rec to_thunk_ty (ty : A.typ) : A.typ = A.FUNCTION_TY (A.UNIT_TY, transform_ty ty)
and transform_ty ty = match ty with 
...
let rec lazy_def def = match def with 
  | A.Function (ty, funname, args, body) -> 
    let lazy_tau = to_thunk_ty ty in 
    A.Function (lazy_tau, funname, ["unit"], lazy_expr (A.Lambda (ty, args, body)))
...
let lazy_convert (program: A.def list) : A.def list = 
  List.map lazy_def program
