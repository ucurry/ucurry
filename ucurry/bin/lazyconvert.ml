...
let rec transform_funty args ty  = 
 ...
 | _ :: xs, A.FUNCTION_TY (arg_tau, ret_tau) ->  
  A.FUNCTION_TY (A.FUNCTION_TY (UNIT_TY, arg_tau), transform_funty xs ret_tau)
...
let rec lazyExpWith ((ty, exp) : S.sexpr) : L.sexpr =
    ...
    | S.SBinop (e1, binop, e2) -> (ty, L.Binop (lazyExpWith e1, binop, lazyExpWith e2))
    | S.SBegin es -> (ty, L.Begin (List.map lazyExpWith es))
    | S.SIf (e1, e2, e3) -> (ty, L.If (lazyExpWith e1, lazyExpWith e2, lazyExpWith e3))
    | S.SLet (bs, e) ->
