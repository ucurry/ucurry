...
-- If we remove this semantics check (line 88 in `clconvert.ml`), the llvm
-- will failed to modified the `x` in the closure because we do not generate
-- a load instruction for `x` in the closure.

-- We are able to mutate local variables in the closure (the ones created by
-- a lambda or a let (see `set-local.uc`)) becuase their heap location is known (its just the input
-- parameter).
